...
- HANA_Tests_StringOperations

[DESCRIPTION]

- Runtime test for string operations
...
- Can be executed to measure performance of string operations
...
BASIS_INFO AS
( SELECT
    'ZMF_TEST_STRING_OPERATION' SQL_CACHE_TAG
  FROM
    DUMMY
),
DIGITS AS
...
WHERE
  '%' || A.DIGIT || B.DIGIT || C.DIGIT || D.DIGIT || E.DIGIT || F.DIGIT || '%' LIKE '123456'
