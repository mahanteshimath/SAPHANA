...
( SELECT
  ...
  ( SELECT                                                      /* Modification section */
      ...
      -1 MIN_ALLOCS_PER_S,
      'HISTORY' DATA_SOURCE,
      'TIME' AGGREGATE_BY,        /* TIME, HOST, PORT, ALLOCATOR or comma separated combinations, NONE for no aggregation */
...
( SELECT
  ...
  ( SELECT
    ...
    SELECT
      ...
      H.CATEGORY ALLOCATOR_NAME,
      CASE
        WHEN H.EXCLUSIVE_ALLOCATED_SIZE  - LAG(H.EXCLUSIVE_ALLOCATED_SIZE,  1) OVER (PARTITION BY H.HOST, H.PORT, H.CATEGORY ORDER BY H.SERVER_TIMESTAMP ) < 0 THEN H.EXCLUSIVE_ALLOCATED_SIZE
        ELSE H.EXCLUSIVE_ALLOCATED_SIZE  - LAG(H.EXCLUSIVE_ALLOCATED_SIZE,  1) OVER (PARTITION BY H.HOST, H.PORT, H.CATEGORY ORDER BY H.SERVER_TIMESTAMP )
      END EXCLUSIVE_ALLOCATED_SIZE,
      CASE
        WHEN H.EXCLUSIVE_ALLOCATED_COUNT - LAG(H.EXCLUSIVE_ALLOCATED_COUNT, 1) OVER (PARTITION BY H.HOST, H.PORT, H.CATEGORY ORDER BY H.SERVER_TIMESTAMP ) < 0 THEN H.EXCLUSIVE_ALLOCATED_COUNT
        ELSE H.EXCLUSIVE_ALLOCATED_COUNT - LAG(H.EXCLUSIVE_ALLOCATED_COUNT, 1) OVER (PARTITION BY H.HOST, H.PORT, H.CATEGORY ORDER BY H.SERVER_TIMESTAMP )
