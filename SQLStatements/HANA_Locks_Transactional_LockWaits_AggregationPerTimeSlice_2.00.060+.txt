...
- HANA_Locks_Transactional_LockWaits_AggregationPerTimeSlice_2.00.060+
...
- 2022/05/26:  1.1 (dedicated 2.00.060+ version including SITE_ID for data source HISTORY)
...
( SELECT DISTINCT
  ...
  ( SELECT DISTINCT
      ...
      LPAD(SUM(NUM_SAMPLES) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID), 13) SAMPLES_TOTAL,
      NTH_VALUE(VALUE, 1) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) DETAIL_1,
      NTH_VALUE(NUM_SAMPLES, 1) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) SAMPLES_1,
      TO_DECIMAL(ROUND(NTH_VALUE(PERCENT, 1) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC)), 10, 0) PCT_1,
      NTH_VALUE(VALUE, 2) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) DETAIL_2,
      NTH_VALUE(NUM_SAMPLES, 2) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) SAMPLES_2,
      TO_DECIMAL(ROUND(NTH_VALUE(PERCENT, 2) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC)), 10, 0) PCT_2,
      NTH_VALUE(VALUE, 3) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) DETAIL_3,
      NTH_VALUE(NUM_SAMPLES, 3) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) SAMPLES_3,
      TO_DECIMAL(ROUND(NTH_VALUE(PERCENT, 3) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC)), 10, 0) PCT_3,
      NTH_VALUE(VALUE, 4) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) DETAIL_4,
      NTH_VALUE(NUM_SAMPLES, 4) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) SAMPLES_4,
      TO_DECIMAL(ROUND(NTH_VALUE(PERCENT, 4) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC)), 10, 0) PCT_4,
      NTH_VALUE(VALUE, 5) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) DETAIL_5,
      NTH_VALUE(NUM_SAMPLES, 5) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) SAMPLES_5,
      TO_DECIMAL(ROUND(NTH_VALUE(PERCENT, 5) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC)), 10, 0) PCT_5,
      NTH_VALUE(VALUE, 6) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) DETAIL_6,
      NTH_VALUE(NUM_SAMPLES, 6) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) SAMPLES_6,
      TO_DECIMAL(ROUND(NTH_VALUE(PERCENT, 6) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC)), 10, 0) PCT_6,
      NTH_VALUE(VALUE, 7) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) DETAIL_7,
      NTH_VALUE(NUM_SAMPLES, 7) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) SAMPLES_7,
      TO_DECIMAL(ROUND(NTH_VALUE(PERCENT, 7) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC)), 10, 0) PCT_7,
      NTH_VALUE(VALUE, 8) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) DETAIL_8,
      NTH_VALUE(NUM_SAMPLES, 8) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) SAMPLES_8,
      TO_DECIMAL(ROUND(NTH_VALUE(PERCENT, 8) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC)), 10, 0) PCT_8,
      NTH_VALUE(VALUE, 9) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) DETAIL_9,
      NTH_VALUE(NUM_SAMPLES, 9) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) SAMPLES_9,
      TO_DECIMAL(ROUND(NTH_VALUE(PERCENT, 9) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC)), 10, 0) PCT_9,
      NTH_VALUE(VALUE, 10) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) DETAIL_10,
      NTH_VALUE(NUM_SAMPLES, 10) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC) SAMPLES_10,
      TO_DECIMAL(ROUND(NTH_VALUE(PERCENT, 10) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID ORDER BY NUM_SAMPLES DESC)), 10, 0) PCT_10,
      MIN_SAMPLES_TOTAL
    FROM
    ( SELECT
        SNAPSHOT_TIME,
        ...
        VALUE,
        COUNT(*) NUM_SAMPLES,
        COUNT(*) / SUM(COUNT(*)) OVER (PARTITION BY SNAPSHOT_TIME, SITE_ID) * 100 PERCENT,
        MIN_SAMPLES_TOTAL
      FROM
      ( SELECT
        ...
        ( SELECT
          ...
          ( SELECT                                                      /* Modification section */
              ...
              -1  MIN_WAIT_TIME_S,
              600 TIME_SLICE_S,
              'TABLE' AGGREGATE_BY     /* TIME, SITE_ID, HOST, PORT, SCHEMA, TABLE, TYPE, MODE, BLOCKING_HOST_PID, BLOCKING_UTID, BLOCKING_CONN_ID, BLOCKING_APP_USER, BLOCKING_APP_SOURCE, BLOCKING_APP_NAME,
                                         BLOCKING_HASH */
            FROM
              DUMMY
          )
