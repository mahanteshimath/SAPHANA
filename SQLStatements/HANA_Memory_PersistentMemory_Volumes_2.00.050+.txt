...
- HANA_Memory_PersistentMemory_Volumes_2.00.050+
...
- M_TABLE_PERSISTENT_MEMORY_FILES available with SAP HANA >= 2.00.050
...
- 2020/06/02:  1.2 (dedicated 2.00.050+ version without HEXSTR2INT function call)

[INVOLVED TABLES]

- M_PERSISTENT_MEMORY_VOLUMES
- M_PERSISTENT_MEMORY_VOLUME_DATA_FILES
- M_TABLE_PERSISTENT_MEMORY_FILES
...
( SELECT
  ...
  ( SELECT
    ...
    ( SELECT
        ...
        IFNULL(V.COLUMN_NAME, '') COLUMN_NAME
      FROM
        M_PERSISTENT_MEMORY_VOLUME_DATA_FILES F LEFT OUTER JOIN
        M_TABLE_PERSISTENT_MEMORY_FILES V ON
          V.FILE_NAME = F.FILE_NAME AND
          V.HOST = F.HOST AND
          V.PORT = F.PORT
    ) F
    WHERE
